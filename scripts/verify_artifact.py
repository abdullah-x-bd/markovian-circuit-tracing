...
def main() -> None:
    ...
    args = p.parse_args()
    errors = [f"missing {name}" for name in REQUIRED_AGGREGATES if not (args.root / name).exists()]
    raw_path = args.root / "raw_runs.json"
    if raw_path.exists():
        raw = json.loads(raw_path.read_text())
        if raw.get("artifact_schema_version") != "1.0":
            errors.append("bad compact run-index schema")
        found = {(r["observability"], int(r["seed"])) for r in raw.get("runs", [])}
        expected = {(o, s) for o in EXPECTED_OBS for s in EXPECTED_SEEDS}
        if found != expected:
            errors.append(f"canonical run grid mismatch: missing={sorted(expected-found)}, extra={sorted(found-expected)}")
    if errors:
    ...
    (args.root / "MANIFEST.json").write_text(json.dumps({"artifact_schema_version":"1.0","canonical_run_count":15,"files":manifest}, indent=2))
